...
`app/signature.tsx` writes "Emmanuel" across the footer as it scrolls into
view, and unwrites it on the way back up.

The word is outlines from Ms Madi (SIL OFL), generated by
`tools/make-signature.py` into `app/signature-path.json`, so no font is
fetched at runtime. It is revealed through a mask rather than a wipe: one thick
